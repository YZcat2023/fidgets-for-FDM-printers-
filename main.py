...
def get_preset_values():
    ...
    labels = ["自定义收起G代码", "自定义放下G代码", "喷嘴笔尖高度差", "最高移动速度", "擦嘴代码", "X坐标补偿值",
              "Y坐标补偿值"]
    entries = []
...
def environment_check():  # 后处理前的操作]
    ...
    for file in os.listdir():
        if "con.ini" in file:  # 使用 'in' 来检查子字符串是否存在于文件名中
            valid_con = True
    ...
    for file in os.listdir():
        ...
        if file.find(".g") != -1 or file.find(".x3g") != -1 or file.find(".gcode") != -1:  # 识别.G,.GCODE,即输入文件
            ...
            if file.find("Output") == -1:
                ...
                # break
            # else:
            #     tk.messagebox.showerror(title='错误', message='程序所在文件夹下已有输出Gcode文件')
            #     sys.exit("Already exists:已有Gcode")
...
def merge_files(file1, file2,temp_file_name,CustomLock,CustomRelease):
    with open(file1, 'r',encoding="utf-8", errors='ignore') as f1, open(file2, 'r',encoding="utf-8", errors='ignore') as f2:
        ...
        # 创建一个字典，用于存储第二个文件中以"LOC"开头的行及其后续数据
        loc_dict = {}
        current_loc = None
        for line in lines2:
...
def main():
    ...
    with open(Input_Filename, "r",encoding="utf-8", errors='ignore') as f:  # 读取输入文件,此过程粗处理文件
        for line in f.readlines():
            ...
            if Copy_Flag==True:
                ...
                if line.find(";WIPE_START")!=-1:#如果发现了开始擦嘴的指令
                    LST_Flag = True
                    for index in range(len(InterFace)):
                        print(InterFace[index],file=TempExporter)
                    InterFace.clear()#输出并清空已捕获的内容
                    AreaCount+=1#区域计数加一
                    print(";LOC" + str(LayerCount) + "B" + str(AreaCount), file=TempExporter)#向临时文件与导出的G code中 输出当前的loc
                    print(";LOC" + str(LayerCount) + "B" + str(AreaCount), file=RawExporter)

                if line.find(";WIPE_END")!=-1:
                    print("G1 Z" + str(round(Current_Layer_Height + Z_Offset, 3)) + ";Keep thisWPE", file=TempExporter)

                if line.find(";TYPE:Inner wall") != -1 or line.find(";TYPE:Outer wall") != -1 or (line.find(";TYPE:Support")!=-1 and line.find("interface")==-1):  # 如果发现了type的类型的变换
    ...
    with open(temp_file_name+".tmp1", "r",encoding="utf-8", errors='ignore') as f:  # 读取输入文件,此过程粗处理文件
        for line in f.readlines():
            line = line.strip('\n')  # 去掉换行符
